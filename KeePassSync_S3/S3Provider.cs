	...
	public class S3Provider : IOnlineProvider
	{
		...
		public override KeePassSyncErr PutFile(PwEntry entry, string remoteFilename, string localFilename)
		{
			...
			try
			{
				...
				reader.Close();
				MetadataEntry[] meta_data = new MetadataEntry[1];
				...
				if (m_UserControl.CreateBackups)
				{
					...
					backupFilename = remoteFilename + ".bkup_day" + DateTime.Today.Day.ToString();

					GenerateSigElements("GetObject", out stamp, out sig);
					try{

						GetObjectResult cur_item = client.GetObject(m_UserControl.BucketName, remoteFilename, false, false, false, m_UserControl.AccessKey, stamp, true, sig, null);
						if (cur_item != null)
						{
							GenerateSigElements("CopyObject", out stamp, out sig);
							client.CopyObject(m_UserControl.BucketName, remoteFilename, m_UserControl.BucketName, backupFilename, MetadataDirective.COPY, true, null, null, System.DateTime.MinValue, false, System.DateTime.MinValue, false, null, null, StorageClass.STANDARD, false, m_UserControl.AccessKey, stamp, true, sig, null);
						}
					}
				...
				var result = client.PutObjectInline(m_UserControl.BucketName, remoteFilename, null, data, data.Length, null, StorageClass.STANDARD, true, m_UserControl.AccessKey, stamp, true, sig, null);
				if (result == null)
		...
		public override KeePassSyncErr GetFile(PwEntry entry, string remoteFilename, string localFilename)
		{
			...
			try
			{
				...
				GenerateSigElements("GetObject", out stamp, out sig);
				var result = client.GetObject(m_UserControl.BucketName, remoteFilename, false, true, true, m_UserControl.AccessKey, stamp, true, sig, null);
		...
		private KeePassSyncErr verify_bucket_or_create(String bucket_name)
		{
			...
			try
			{
				...
				var result = client.ListAllMyBuckets(m_UserControl.AccessKey, stamp, true, sig);
				...
				var bres = client.CreateBucket(bucket_name, null, m_UserControl.AccessKey, stamp, true, sig);
